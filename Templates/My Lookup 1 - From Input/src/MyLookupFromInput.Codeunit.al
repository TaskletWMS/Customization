codeunit 70010 "MyLookup From Input"
{
    // Template: Lookup function with user-input header — the user fills in search criteria, and a filtered list is returned.
    //
    // Flow: User selects the menu item → fills in header search fields → accepts →
    //       device fetches the filtered list → result rows are displayed.
    //
    // Implements:
    //   Distribute Tweak     — sends the tweak XML to the mobile device on login
    //   Define Header Fields — defines the fields the user fills in before the list is loaded
    //   Handle Lookup        — returns the filtered list based on the header values
    ...
    // HANDLE LOOKUP
    //
    // Called when the device requests the lookup list after the header is accepted.
    ...
    begin
        ...
        MySearchField := Samples.ReadSampleHeaderValue(_RequestValues);
        Samples.AddSampleLookupRows(_LookupResponseElement, MySearchField);
        _IsHandled := true;
    end;
    ...
    begin
        ...
        Samples.CreateSampleMenuOption('MyLookupFromInput', 'WMS');
    end;
